...
// [ok] 图像点的运算通常涉及到读取、修改和处理图像中每个像素点的颜色值
func case1() {
	filePath := "./test.png"
...
// [ok] case5 图像减法可以用于检测图像中的变化或突出差异
func case5() {
	img1 := getTestImg()
	img2 := getTest2Img()
...
// [ok] case8 图像的逻辑运算通常指的是对图像像素进行按位逻辑操作，例如与（AND）、或（OR）、异或（XOR）和非（NOT）操作。
...
// [ok] case9 图像缩放  使用 golang.org/x/image/draw 包实现图像缩放
...
// [ok] case21 图像的刚性变换，仿射变换，透视变换，添加双线性插值等抗锯齿技术
...
// [todo] case62 计算图片的RGB分量矩阵 - 打印矩阵，终端输出二维数值
...
// [todo] case63 打印图片的灰度矩阵，在终端显示二维数值
...
// [todo] case64 打印图片的二值矩阵，在终端显示二维数值
